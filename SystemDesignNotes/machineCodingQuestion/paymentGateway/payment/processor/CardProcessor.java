...
public class CardProcessor  extends Processor{
    @Override
    public Payment validateAndCreate(PaymentDao doa) {

        // do card some validation
        ...
        pay.setType(PaymentType.CARD);


        payments.put(pay.getPaymentId(), pay) ;


        System.out.println(" Card Payment ValidateAndCreate step : " + pay.getPaymentId()) ;
        return pay ; 
    }

    @Override
    public  void initiate(int paymentId) {
        System.out.println("Card Payment Initiated step : " + paymentId) ;
    }
    @Override
    public  void authenticate(int paymentId) {
        System.out.println("Card Payment Payment Authenticated step : " + paymentId) ;
        payments.get(paymentId).setStatus(PaymentStatus.AUTHENTICATED);
    }
    @Override
    public  void authorized(int payId) {
        System.out.println("Card Payment Payment authorized step : " + payId) ;
        payments.get(payId).setStatus(PaymentStatus.AUTHORIZED);
    }
    @Override
    public  void capture(int payId) {
        System.out.println("Card Payment Payment capture step : " + payId) ;
