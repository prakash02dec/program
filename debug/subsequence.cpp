...
// 	   for(int i {} , j {} , k {}  ; i<N , j < zero.size() , k<zero.size()  ; i++){
// 	        int total_seq {};
//             if(i%2==0)
// 	            optimal.push_back(zero.at(j++));
// 	        else
// 	            optimal.push_back(one.at(k++));
	       
// 	        std::for_each(zero.begin() , zero.end() , [&total_seq , i , one](int zeros)
// 	        {total_seq += zeros * std::accumulate(one.begin() + i, one.end() , 0 ) ;});
// 	   }
// 	   for(const auto &ele : optimal){
// 	       std::cout<<ele << " ";
// 	   }
