...
void solve() {
   ...
   if(2*t+l>=p && n >= 8){
    ...
    int total_task_have {(n-1)/7 +1} ;

    int no_task_we_need {ceil((double)p/t)} ;
    // int task_we_need {(double)p/(2*t+l)} ;
    if(total_task_have >= no_task_we_need  ){
        int days {0} ;
        while(p>0){
            p -= (2*t + l );
            days++ ;
    ...
    int days {} ;
    p -= (total_task_have)*t ;
    if(total_task_have%2 == 0){
        p -= (total_task_have/2)*l ;
        days += (total_task_have/2) ;
    }else{
        p -= (total_task_have/2 + 1)*l ;
        days += (total_task_have/2 +1)  ;
    }
    // cout << days << endl ;
    if(p>0){
    int left_points {ceil((double)p/l)} ;
