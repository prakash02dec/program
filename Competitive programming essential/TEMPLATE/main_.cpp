#include <bits/stdc++.h>
using namespace std;

#define int            long long int
#define S              second
#define F              first
#define pb             push_back
#define si             set <int>
#define vi             vector <int>
#define pii            pair <int, int>
#define vpi            vector <pii>
#define vpp            vector <pair<int, pii>>
#define mii            map <int, int>
#define mpi            map <pii, int>
#define spi            set <pii>
#define sz(x)          ((int) x.size())
#define double         long double
#define all(p)         p.begin(), p.end()
#define que_max        priority_queue <int>
#define que_min        priority_queue <int, vi, greater<int>>
#define bug(...)       __f (#__VA_ARGS__, __VA_ARGS__)
#define print(a)       for(auto x : a) std::cout << x << " "; std::cout << std::endl
#define print1(a)      for(auto x : a) std::cout << x.F << " " << x.S << std::endl
#define print2(a,x,y)  for(int i = x; i < y; i++) std::cout<< a[i]<< " "; std::cout << endl

const int mod {998244353} , N{2000001};
const int sz {3};

struct Mat {
    long long m[sz][sz] ; 
    Mat() {
        memset(m, 0 , sizeof(m)) ;
    }
    void identity(){
        for(int i {0} ; i < sz ; i++){
            m[i][i] = 1 ;
        }
    }
    Mat operator*(Mat a){
        Mat res {}; 
        for(int i {} ; i < sz ; i++){
            for(int j {} ; j < sz ; j++){
                for(int k {} ; k < sz ; k++ ){
                    res.m[i][j] += m[i][k]*a.m[k][j] ; 
                    res.m[i][j] %= mod ; 
                }
            }
        }
        return res ;
    }
};

inline int power(int a, int b)
{
    int x = 1;
    while (b)
    {
        if (b & 1) x *= a;
        a *= a;
        b >>= 1;
    }
    return x;
} 

int  fac[N] {} ;

int addm(int x , int y) {
    return (x+y) % mod ;
}

int subm(int x , int y){
    return ((x - y ) + mod ) % mod ;
}

int mulm (int x , int y){
    return (x*y) %mod ;
}


int powm(int x , int m ){
    int result {1} ;
    while(m){
        if(m&1) result = mulm(x, result) ;
        x = mulm(x ,x) ;
        m /= 2 ;
    }
    return result ;
}


int inv(int y){
    return  powm(y , mod-2) ;
}

// using fermat theorum
int divm(int x , int y){
    return mulm (x , inv(y)) ;
}

void caculate_factorial() {
    fac[0] = 1 ;
    for(int i=1 ; i< N ; i++){
        fac[i] = mulm(fac[i-1] ,i) ;
    }

}

int nCr(int n , int r){
    return mulm (mulm(fac[n] , inv(fac[r])) , inv(fac[n-r])) ;
}

template <typename Arg1>
void __f (const char* name, Arg1&& arg1) { std::cout << name << " : " << arg1 << std::endl; }
template <typename Arg1, typename... Args>
void __f (const char* names, Arg1&& arg1, Args&&... args)
{
    const char* comma = strchr (names + 1, ',');
    std::cout.write (names, comma - names) << " : " << arg1 << " | "; __f (comma + 1, args...);
}


struct fenwick {
	vector<int> fn;
	int n;
	fenwick () {} 
	fenwick (int n) {init(n) ; } 

	void init(int n) {
		this->n = n + 1;
		fn.resize(this->n, 0);
	}

	void add(int x, int y) {
		// x++; // 1 based index
		while (x < n) {
			fn[x] += y;
			x += (x & (-x));// last set bit
		}
	}

	// void add(int x, int y) {
	// 	for (x++; x < n; x += (x & (-x))) fn[x] += y;
	// }

	int sum(int x) {
// 		x++; // 1 based index
		int ans = 0;
		while (x) {
			ans += fn[x];
			x -= (x & (-x));// last set bit
		}
		return ans;
	}

	int sum(int l, int r) {
		return sum(r) - sum(l - 1);
	}

};

vector<long long> solve(int n,int m, vector<int> a, vector<vector<int>> query){
    vector<long long> counts (static_cast<int>(query.size()) ,0 )  ;
    unordered_map<int , int> hash , right_most;
    
    
    int size {static_cast<int>(a.size()) } ;
    
    for(int i {} ; i < query.size() ; i++){
        query[i].push_back(i) ;
    }
    sort(query.begin() , query.end() , [](auto q1 , auto q2){
       if(q1[1] == q2[1]) return q1[0] < q2[0] ;
       return q1[1] < q2[1] ;
    });


    fenwick tree (size) ;

    int x {1} , y {1} ;
    int q {0} ;

    while(q < query.size()){
        // y move right add element
        while(y <= query[q][1] ){
            if(hash[a[y-1]] != 0)
                tree.add(right_most[a[y-1]] , -hash[a[y-1]]*hash[a[y-1]]*a[y-1] ) ;
            
            hash[a[y-1]]++ ;
            right_most[a[y-1]] = y ;
                
            tree.add( y , hash[a[y-1]]*hash[a[y-1]]*a[y-1] ) ;

            y++ ;

        }
        // x move right add element
        while(x < query[q][0] ){
            
            tree.add(right_most[a[x-1]] , -hash[a[x-1]]*hash[a[x-1]]*a[x-1]) ;
            hash[a[x-1]]-- ;
            
            if(hash[a[x-1]] != 0)
                 tree.add(right_most[a[x-1]], hash[a[x-1]]*hash[a[x-1]]*a[x-1] ) ;
            x++ ;

        }

        // x move left add element
        while(x > query[q][0] ){
            x-- ;
            if(hash[a[x-1]] != 0)
                tree.add(right_most[a[x-1]] , -hash[a[x-1]]*hash[a[x-1]]*a[x-1])   ;
            
            hash[a[x-1]]++ ;
                
            tree.add(right_most[a[x-1]] ,  hash[a[x-1]]*hash[a[x-1]]*a[x-1] ) ;
        }

        counts[query[q][2]] = tree.sum(query[q][0] , query[q][1]) ;

        q++ ;
        
    }

    return counts ;
}


int32_t main()
{
    std::ios_base::sync_with_stdio(0); std::cin.tie(0); std::cout.tie(0);

#ifndef ONLINE_JUDGE
    freopen("input.txt",  "r",  stdin);
    freopen("output.txt", "w", stdout);
#endif

    clock_t z = clock();

    int t = 1;
    // caculate_factorial() ;
    // std::cin >> t;
    while (t--) {
        int n {} ;
        cin >> n ;        
        vector<int> a  ;
        for(int i{ } ; i<n ; i++) {
            int temp {} ;
            cin >> temp ;
            a.push_back(temp) ;
        }
        int q {} ;
        cin >> q ;
        vector<vector<int>> queries  ;
        for(int i{ } ; i<q ; i++) {
            int l {} , r{}  ;
            cin >> l >> r ;
            queries.push_back(vector<int> {l , r}) ;
        }
        solve(n ,q , a , queries);
    }
    
    // std::cerr << "Run Time : " << ((double)(clock() - z) / CLOCKS_PER_SEC);

    return 0;
}


/*
6
1 2 2 3 3 3 
1
1 2
2 2
1 3
2 4
4 6
1 6

*/